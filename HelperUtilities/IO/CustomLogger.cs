...
namespace HelperUtilities.IO
{
    public class Logger
    {
        ...
        public Logger(string referenceId = null)
        {
        ...
        public Logger(string baseDirectoryWithAbsolutePath, string fileNameWithExtensionForLog, string FileNameWithExtensionForErrors, string referenceId = null)
        {
        ...
        public void AppendLog(string logText, object obj = null)
        {
            if (_sbLog.Length == 0)
            {
                _sbLog.AppendLine($"{_referenceId} - {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss.fff",CultureInfo.InvariantCulture)}");
            }
            _sbLog.AppendLine($"\t{logText} ({DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss.fff", CultureInfo.InvariantCulture)})");
            if (obj != null)
            {
